...
このセクションでは、システムで使用する各APIの詳細な設定方法を説明します。
...
- **gmail_client.py**: Gmail APIとの通信を行い、メールの取得と送信を実装します。
- **email_processor.py**: メールの解析、添付ファイルの処理、メールヘッダーの解析などを行います。

### discord_module

Discordボットの操作とメッセージ処理を担当します。

- **discord_bot.py**: Discordボットの実装、コマンド処理、ボタン処理などを行います。
- **message_formatter.py**: Discordメッセージのフォーマット、埋め込み、ボタンの作成などを担当します。

### ai_module

AIプロバイダー（ChatGPTとClaude）の切り替えを担当します。

- **ai_factory.py**: ファクトリーパターンを使用して、設定に基づいて適切なAIプロバイダーのインスタンスを生成します。

### chatgpt_module / claude_module

各AIプロバイダーとの連携を担当します。

- **response_processor.py**: AIからの応答を処理し、XML形式の出力を解析します。

### calendar_module

Googleカレンダーとの連携を担当します。

- **calendar_client.py**: Googleカレンダー APIとの通信を行い、予定の取得を実装します。
- **schedule_analyzer.py**: 取得した予定を分析し、利用可能なスロットを特定します。

### name_module

送信者情報からの宛名抽出と管理を担当します。

- **name_manager.py**: 宛名情報の保存、取得、更新を管理します。

### utils

ロギングやAI出力の保存などのユーティリティ機能を担当します。

- **logger.py**: ログ機能を提供します。
- **output_saver.py**: AI出力を保存し、後で参照できるようにします。

### config

設定ファイルとプロンプトの管理を担当します。

- **config.py**: 環境変数や設定ファイルの読み込みと管理を行います。
...
このセクションでは、システムを拡張するためのガイドラインを提供します。

### 新しいAIプロバイダーの追加

新しいAIプロバイダーを追加するには、以下の手順に従ってください：

1. `ai_module/ai_factory.py`に新しいプロバイダーのケースを追加
2. 新しいプロバイダー用のモジュールディレクトリを作成（例：`new_ai_module/`）
3. `response_processor.py`を実装し、共通インターフェースに準拠
4. `.env`ファイルに新しいプロバイダーのAPIキー設定を追加

### カスタムプロンプトの作成

システムプロンプトをカスタマイズするには、以下のファイルを編集します：

1. `gmail_discord_bot/config/email_analyzer_prompt.txt` - メール分析用プロンプト
2. `gmail_discord_bot/config/email_responder_prompt.txt` - 返信生成用プロンプト

プロンプトを編集する際は、XML形式の出力構造を維持してください。

### 新機能の追加

新機能を追加する場合は、以下の点に注意してください：

1. モジュール構造に従って適切な場所に実装
2. 既存のインターフェースとの互換性を維持
3. 適切なエラーハンドリングとロギングを実装
4. ユニットテストを作成して機能を検証

## 🔍 トラブルシューティング

このセクションでは、開発中によく発生する問題とその解決方法を説明します。
